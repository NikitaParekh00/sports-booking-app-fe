            ...
            .on('postgres_changes', {
                ...
                if (payload.new) {
                    ...
                    if (isUpdatingIndexRef.current) {
                        ...
                        // Query fresh data to ensure we have the latest skipped players (same approach as unbidded mode)
                        // Only update playersRef.current, don't call setPlayers() to avoid triggering effects that reset bid
                        ...
                        if (skippedPlayersData && skippedPlayersData.length > 0) {
                            ...
                            // Only update ref, not state - this prevents triggering effects that reset bid
                            playersRef.current = skippedFromTable;
                            // Only update state if the list actually changed (to avoid unnecessary re-renders)
                            if (players.length !== skippedFromTable.length || 
                                players.some((p, i) => p.name !== skippedFromTable[i]?.name)) {
                                setPlayers(skippedFromTable);
                                setSkippedPlayers(skippedFromTable);
                            }
                        } else {
